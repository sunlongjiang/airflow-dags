...
CREATE EXTERNAL TABLE hungry_studio.dws_block_blast_gp_block_action_game_di(
...
COMMENT 'block blast gp 游戏-局粒度汇总数据'
PARTITIONED BY ( 
  dt string)
ROW FORMAT SERDE 
...
LOCATION
  's3://hungry-studio-data-warehouse/dws/block_blast/gp/block_action_game_di'
TBLPROPERTIES (
  'classification'='parquet', 
  'transient_lastDdlTime'='1701767686')

ALTER TABLE hungry_studio.dws_block_blast_gp_block_action_game_di ADD COLUMNS (time_diff_in_seconds double COMMENT '落块-整体时间'); 
ALTER TABLE hungry_studio.dws_block_blast_gp_block_action_game_di ADD COLUMNS (time_action_in_seconds double COMMENT '落块-动作时间');
ALTER TABLE hungry_studio.dws_block_blast_gp_block_action_game_di ADD COLUMNS (time_think_in_seconds double COMMENT '落块-思考时间');
ALTER TABLE hungry_studio.dws_block_blast_gp_block_action_game_di ADD COLUMNS ( roundinfo array<array<string>> COMMENT '轮信息流汇总')


-- 20240826  zhaodawei  新增复活等指标
ALTER TABLE hungry_studio.dws_block_blast_gp_block_action_game_di ADD COLUMNS (inter_ad_pv bigint COMMENT '插屏广告展示次数');
ALTER TABLE hungry_studio.dws_block_blast_gp_block_action_game_di ADD COLUMNS (rewarded_ad_pv bigint COMMENT '激励广告展示次数');
ALTER TABLE hungry_studio.dws_block_blast_gp_block_action_game_di ADD COLUMNS (banner_ad_pv bigint COMMENT 'banner广告展示次数');
ALTER TABLE hungry_studio.dws_block_blast_gp_block_action_game_di ADD COLUMNS (inter_ad_revenue double COMMENT '插屏广告展示收入');
ALTER TABLE hungry_studio.dws_block_blast_gp_block_action_game_di ADD COLUMNS (rewarded_ad_revenue double COMMENT '激励广告展示收入');
ALTER TABLE hungry_studio.dws_block_blast_gp_block_action_game_di ADD COLUMNS (banner_ad_revenue double COMMENT 'banner广告展示收入');
ALTER TABLE hungry_studio.dws_block_blast_gp_block_action_game_di ADD COLUMNS (real_time bigint COMMENT '游戏时长');
ALTER TABLE hungry_studio.dws_block_blast_gp_block_action_game_di ADD COLUMNS (weight double COMMENT '权重');
ALTER TABLE hungry_studio.dws_block_blast_gp_block_action_game_di ADD COLUMNS (blast_cnt bigint COMMENT '消除次数');
ALTER TABLE hungry_studio.dws_block_blast_gp_block_action_game_di ADD COLUMNS (clean_cnt bigint COMMENT '消除行列数');
ALTER TABLE hungry_studio.dws_block_blast_gp_block_action_game_di ADD COLUMNS (clear_screen_cnt bigint COMMENT '清屏次数');
ALTER TABLE hungry_studio.dws_block_blast_gp_block_action_game_di ADD COLUMNS (revive_round_id_list array<String> COMMENT '复活轮id列表');
ALTER TABLE hungry_studio.dws_block_blast_gp_block_action_game_di ADD COLUMNS (all_puzzle_round_id_list array<String> COMMENT '遇到死亡难题的轮id列表(三个策略均为死亡难题)');
ALTER TABLE hungry_studio.dws_block_blast_gp_block_action_game_di ADD COLUMNS (dead_reason int COMMENT '死亡原因,1:死亡难题,2:故意怼死, -1:其他');
ALTER TABLE hungry_studio.dws_block_blast_gp_block_action_game_di ADD COLUMNS (is_sdk_sample int comment '是否抽样');


ALTER TABLE hungry_studio.dws_block_blast_gp_block_action_game_di ADD COLUMNS (session_id int COMMENT '每次冷启，截止下一次冷启');
ALTER TABLE hungry_studio.dws_block_blast_gp_block_action_game_di ADD COLUMNS (combo5_cnt string COMMENT '到combo5的次数');
ALTER TABLE hungry_studio.dws_block_blast_gp_block_action_game_di ADD COLUMNS (combo10_cnt string COMMENT '到combo10的次数');
ALTER TABLE hungry_studio.dws_block_blast_gp_block_action_game_di ADD COLUMNS (combo15_cnt string COMMENT '到combo15的次数');
ALTER TABLE hungry_studio.dws_block_blast_gp_block_action_game_di ADD COLUMNS (matrix_before_dead string COMMENT '死亡前本轮的初始盘面');
ALTER TABLE hungry_studio.dws_block_blast_gp_block_action_game_di ADD COLUMNS (block_list_before_dead string COMMENT '死亡前本轮的出题');
ALTER TABLE hungry_studio.dws_block_blast_gp_block_action_game_di ADD COLUMNS (game_popup_revive_click_cnt string COMMENT '本局复活弹窗 点击次数');
ALTER TABLE hungry_studio.dws_block_blast_gp_block_action_game_di ADD COLUMNS (inter_ad_request_pv string COMMENT '本局 插屏请求次数');
ALTER TABLE hungry_studio.dws_block_blast_gp_block_action_game_di ADD COLUMNS (rewarded_ad_request_pv string COMMENT '本局 激励请求次数');


ALTER TABLE hungry_studio.dws_block_blast_gp_block_action_game_di ADD COLUMNS (init_matrix string COMMENT '本局初始牌面');

"""

if __name__ == "__main__":
    # 创建 Spark 会话
    spark = SparkSession.builder.appName("block_blast_gp_block_action_game_di").enableHiveSupport().getOrCreate()

    DT = sys.argv[1]

    # 读取原始数据
    sdk_user_info_source = "hungry_studio.dim_block_blast_gp_user_ha"
    user_label_source = "hungry_studio.dim_block_blast_gp_user_label_ha"
    game_source = "hungry_studio.dwd_block_blast_gp_block_action_game_di"
    round_source = "hungry_studio.dws_block_blast_gp_block_action_round_di"
    white_event_source = "hungry_studio.dwd_block_blast_gp_white_event_unique_data_hi"
    target_table  = "hungry_studio.dws_block_blast_gp_block_action_game_di"
    ...
    white_event_query = f"""
             select
                       ...
                       ,sum(if(event_name in ('admob_sdk_ad_revenue', 'ironSource_sdk_postbacks', 'appLovin_sdk_ad_revenue', 'hs_sdk_ad_revenue') and LOWER(get_json_object(properties, '$.adFormat')) = 'inter', 1, 0))                                                            as inter_ad_pv
                       ,sum(if(event_name in ('admob_sdk_ad_revenue', 'ironSource_sdk_postbacks', 'appLovin_sdk_ad_revenue', 'hs_sdk_ad_revenue') and LOWER(get_json_object(properties, '$.adFormat')) = 'rewarded', 1, 0))                                                         as rewarded_ad_pv
                       ,sum(if(event_name in ('hs_ad_banner_revenue'), 1, 0))                                                                                                                                                                                                       as banner_ad_pv
                       ,sum(if(event_name in ('admob_sdk_ad_revenue', 'ironSource_sdk_postbacks', 'appLovin_sdk_ad_revenue', 'hs_sdk_ad_revenue') and LOWER(get_json_object(properties, '$.adFormat')) = 'inter', cast(get_json_object(properties, '$.revenue') as double), 0))     as inter_ad_revenue
                       ,sum(if(event_name in ('admob_sdk_ad_revenue', 'ironSource_sdk_postbacks', 'appLovin_sdk_ad_revenue', 'hs_sdk_ad_revenue') and LOWER(get_json_object(properties, '$.adFormat')) = 'rewarded', cast(get_json_object(properties, '$.revenue') as double), 0))  as rewarded_ad_revenue
                       ...
                       ,sum(if(event_name in ('game_popup_revive_click'), 1, 0))   as game_popup_revive_click_cnt --本局复活弹窗 点击次数 
