...
if __name__ == "__main__":
    ...
    exec_sql = '''
     insert overwrite table hungry_studio.dwd_block_blast_ios_block_action_block_pre_di
		select device_id,
			app_id,
			install_datetime,
			os,
			...
			game_id,
			game_type,
			case when round_id='<null>' then -1 else int(round_id) end round_id,
			travel_id,
			travel_lv,
			matrix,
			position,
			...
			case
				when replace( matrix, '-1', '' ) rlike '[0-9]' then false
				when replace( matrix, '-1', '' ) is null then false else true
            ...
            CASE
			  ...
			  WHEN combo_cnt <> '-1' AND blast_row_col_cnt = 1 THEN (combo_cnt + 2) * 10 + common_block_cnt -- 当处于combo且消除行列数为1时，单次得分为：（combo数+2）*10 + 块的个数
			  WHEN combo_cnt <> '-1' AND blast_row_col_cnt > 1 THEN (combo_cnt + 2) * blast_row_col_cnt *(blast_row_col_cnt -1) * 10 + common_block_cnt -- 当处于combo且消除行列数大于1时，单次得分为：（combo数+2）*消除行列数*（消除行列数-1）*10 + 块的个数
			  ELSE blast_row_col_cnt * 10 + common_block_cnt --仅完成消除时，单次得分为：行列消除数 * 10 + 块的个数
			END step_score,
			cast(click_rank_per_round as int) block_index_id,
		...
		'''.format(tmp_tbl_name=tmp_tbl_name,begin_id=begin_id,end_id=end_id)
    print(exec_sql)
    spark.sql(exec_sql)
