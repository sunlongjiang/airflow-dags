...
from common.conf import EMR_EC2_JOB_FLOW_ID_DEFAULT,EMR_EC2_JOB_FLOW_ID_DEFAULT_2,EMR_EC2_JOB_FLOW_ID_BLOCK_GP
from airflow.operators.python_operator import PythonOperator
...
execution_date = '{{ (execution_date - macros.timedelta(days=9)).strftime("%Y-%m-%d") }}'
...
BLOCK_CREATE_TABLE_SPARK_STEPS = [
    {
        'Name': 'block_blast_gp_block_action_block_di_create_table',  # 天级别计算app收入聚合数据
        ...
        'HadoopJarStep': {
            ...
            'Args': [
                ...
                "--conf", "spark.driver.memory=5705M",
                "--conf", "spark.executor.memory=5705M", ##约等于每core分配5705M
                ...
                "s3://hungry-studio-airflow/etl/data_warehouse/dwd/block_blast_gp_block_action_block_di_pre_T10_0_create_table.py",
                execution_date,
                hour
            ],
        }
...
BLOCK_PRE_0_1_SPARK_STEPS = [
    {
        'Name': 'block_blast_gp_block_action_block_di_pre_0_1',  # 天级别计算app收入聚合数据
        'ActionOnFailure': 'CONTINUE',
        # 定义了步骤失败时的行为，"TERMINATE_CLUSTER":当步骤失败时，整个EMR集群会终止，"CANCEL_AND_WAIT":失败时取消任何后续步骤并等待，"CONTINUE":即使步骤失败也继续执行后续步骤
        'HadoopJarStep': {
            'Jar': 'command-runner.jar',
            'Args': [
                ...
                "-c", "spark.pyspark.python=./environment/bin/python",
                "--archives", "s3://hungry-studio-airflow/venv_arm/pyspark-airflow-venv.tar.gz#environment",
                "--conf", f"spark.app.name=block_blast_gp_block_action_block_di_pre_0_1_{execution_date}",
                "--conf", "spark.hadoop.hive.exec.dynamic.partition=true",
                "--conf", "spark.hadoop.hive.exec.dynamic.partition.mode=nonstrict",
                "--conf", "spark.sql.legacy.allowNonEmptyLocationInCTAS=true",
                #"--conf", "spark.executor.instances=125",
                "--conf", "spark.driver.memory=22820M",
                "--conf", "spark.executor.memory=22820M", ##约等于每core分配5705M
                ...
                "s3://hungry-studio-airflow/etl/data_warehouse/dwd/block_blast_gp_block_action_block_di_pre_T10_0.py",
                execution_date,
                hour,
                "16",
                "23"
...
BLOCK_PRE_0_2_SPARK_STEPS = [
    {
        'Name': 'block_blast_gp_block_action_block_di_pre_0_2',  # 天级别计算app收入聚合数据
        'ActionOnFailure': 'CONTINUE',
        # 定义了步骤失败时的行为，"TERMINATE_CLUSTER":当步骤失败时，整个EMR集群会终止，"CANCEL_AND_WAIT":失败时取消任何后续步骤并等待，"CONTINUE":即使步骤失败也继续执行后续步骤
        'HadoopJarStep': {
            'Jar': 'command-runner.jar',
            'Args': [
                ...
                "--archives", "s3://hungry-studio-airflow/venv_arm/pyspark-airflow-venv.tar.gz#environment",
                "--conf", "spark.hadoop.hive.exec.dynamic.partition=true",
                "--conf", f"spark.app.name=block_blast_gp_block_action_block_di_pre_0_2_{execution_date}",
                "--conf", "spark.hadoop.hive.exec.dynamic.partition.mode=nonstrict",
                "--conf", "spark.sql.legacy.allowNonEmptyLocationInCTAS=true",
                #"--conf", "spark.executor.instances=125",
                "--conf", "spark.driver.memory=22820M",
...
BLOCK_PRE_0_3_SPARK_STEPS = [
    {
        'Name': 'block_blast_gp_block_action_block_di_pre_0_3',  # 天级别计算app收入聚合数据
        'ActionOnFailure': 'CONTINUE',
        # 定义了步骤失败时的行为，"TERMINATE_CLUSTER":当步骤失败时，整个EMR集群会终止，"CANCEL_AND_WAIT":失败时取消任何后续步骤并等待，"CONTINUE":即使步骤失败也继续执行后续步骤
        'HadoopJarStep': {
            'Jar': 'command-runner.jar',
            'Args': [
                ...
                "-c", "spark.pyspark.python=./environment/bin/python",
                "--archives", "s3://hungry-studio-airflow/venv_arm/pyspark-airflow-venv.tar.gz#environment",
                "--conf", f"spark.app.name=block_blast_gp_block_action_block_di_pre_0_3_{execution_date}",
                "--conf", "spark.hadoop.hive.exec.dynamic.partition=true",
                "--conf", "spark.hadoop.hive.exec.dynamic.partition.mode=nonstrict",
                "--conf", "spark.sql.legacy.allowNonEmptyLocationInCTAS=true",
                #"--conf", "spark.executor.instances=125",
...
GAME_SPARK_STEPS = [
    {
        ...
        'HadoopJarStep': {
            ...
            'Args': [
                ...
                "--conf", "spark.hadoop.hive.exec.dynamic.partition=true",
                "--conf", "spark.hadoop.hive.exec.dynamic.partition.mode=nonstrict",
                "--conf", "spark.driver.memory=17115M",
                "--conf", "spark.executor.memory=17115M",##约等于每core分配5705M
...
DWS_GAME_SPARK_STEPS = [
    {
        ...
        'HadoopJarStep': {
            ...
            'Args': [
                ...
                "--conf", "spark.hadoop.hive.exec.dynamic.partition=true",
                "--conf", "spark.hadoop.hive.exec.dynamic.partition.mode=nonstrict",
                "--conf", "spark.driver.memory=11410M",
                "--conf", "spark.executor.memory=11410M",##约等于每core分配5705M
...
ROUND_REVIVE_SPARK_STEPS = [
    {
        ...
        'HadoopJarStep': {
            ...
            'Args': [
                ...
                "--conf", "spark.driver.memory=22820M",#约等于每core分配5705M
                "--conf", "spark.executor.memory=22820M",
                "--conf", "spark.executor.cores=2",
...
ROUND_SPARK_STEPS = [
    {
        ...
        'HadoopJarStep': {
            ...
            'Args': [
                ...
                "--conf", "spark.sql.legacy.allowNonEmptyLocationInCTAS=true",
                "--conf", "spark.driver.memory=17115M",
                "--conf", "spark.executor.memory=17115M",
                "--conf", "spark.executor.cores=3",
                "--conf", "spark.executor.instances=100",
                "--conf", "spark.default.parallelism=600",
                "--conf", "spark.sql.shuffle.partitions=600",
...
with DAG(
    ...
    dwd_block_blast_gp_block_action_block_di_pre_create_table = EmrAddStepsOperator(
        task_id='dwd_block_blast_gp_block_action_block_di_pre_create_table',
        job_flow_id=EMR_EC2_JOB_FLOW_ID_BLOCK_GP,
        aws_conn_id='aws_default',
        steps=BLOCK_CREATE_TABLE_SPARK_STEPS,
        wait_for_completion=True,
        waiter_delay=120,  # 检测间隔时长
        waiter_max_attempts=43200,  # 检测次数
    )

    dwd_block_blast_gp_block_action_block_di_pre_0_1 = EmrAddStepsOperator(
        task_id='dwd_block_blast_gp_block_action_block_di_pre_0_1',
        job_flow_id=EMR_EC2_JOB_FLOW_ID_BLOCK_GP,
        aws_conn_id='aws_default',
        steps=BLOCK_PRE_0_1_SPARK_STEPS,
        wait_for_completion=True,
        waiter_delay=120,  # 检测间隔时长
        waiter_max_attempts=43200,  # 检测次数
    )

    dwd_block_blast_gp_block_action_block_di_pre_0_2 = EmrAddStepsOperator(
        task_id='dwd_block_blast_gp_block_action_block_di_pre_0_2',
        job_flow_id=EMR_EC2_JOB_FLOW_ID_BLOCK_GP,
        aws_conn_id='aws_default',
        steps=BLOCK_PRE_0_2_SPARK_STEPS,
        wait_for_completion=True,
        waiter_delay=120,  # 检测间隔时长
        waiter_max_attempts=43200,  # 检测次数
    )

    dwd_block_blast_gp_block_action_block_di_pre_0_3 = EmrAddStepsOperator(
        task_id='dwd_block_blast_gp_block_action_block_di_pre_0_3',
        job_flow_id=EMR_EC2_JOB_FLOW_ID_BLOCK_GP,
        aws_conn_id='aws_default',
        steps=BLOCK_PRE_0_3_SPARK_STEPS,
        wait_for_completion=True,
        waiter_delay=120,  # 检测间隔时长
        waiter_max_attempts=43200,  # 检测次数
    )

    all_pre_tasks_completed = DummyOperator(task_id='all_pre_tasks_completed')
    ...
    begin_dag >> dwd_block_blast_gp_block_action_game_di  >> dws_block_blast_gp_block_action_game_di
    # begin_dag >> dwd_block_blast_gp_block_action_block_di_pre_0 >> print_run_date >> [dwd_block_blast_gp_block_action_block_di_pre_1,dwd_block_blast_gp_block_action_block_di_pre_2,dwd_block_blast_gp_block_action_block_di_pre_3] >> dwd_block_blast_gp_block_action_block_di  >> dws_block_blast_gp_block_action_round_di >> create_success_flag >> dws_block_blast_gp_block_action_game_di
    begin_dag >> dwd_block_blast_gp_block_action_block_di_pre_create_table >> [dwd_block_blast_gp_block_action_block_di_pre_0_1,dwd_block_blast_gp_block_action_block_di_pre_0_2,dwd_block_blast_gp_block_action_block_di_pre_0_3] >> all_pre_tasks_completed >> print_run_date >> [dwd_block_blast_gp_block_action_block_di_pre_1,dwd_block_blast_gp_block_action_block_di_pre_2,dwd_block_blast_gp_block_action_block_di_pre_3] >> dwd_block_blast_gp_block_action_block_di  >> [dws_block_blast_gp_block_action_round_di_revive,check_s3_dim_block_blast_gp_user_label_ha_path] >> dws_block_blast_gp_block_action_round_di_pre >> dws_block_blast_gp_block_action_round_di >> create_success_flag >> dws_block_blast_gp_block_action_game_di
