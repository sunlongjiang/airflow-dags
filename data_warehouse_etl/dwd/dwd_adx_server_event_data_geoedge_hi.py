...
if __name__ == "__main__":
    ...
    app_name = f"dwd_adx_server_event_data_geoedge_hi_{date_str}_{hour_str}_{min_str}"
    spark = SparkSession.builder.appName(app_name).enableHiveSupport().getOrCreate()
    print(f"app_name = {app_name}")
    # 声明表名
    table = 'hungry_studio.dwd_adx_server_event_data_geoedge_hi'
    # 定义分区字段的值
    ...
    for bundle_id in bundle_ids:
        for os in os_values:
            partition = f"(bundle_id='{bundle_id}', os='{os}', dt='{date_str}', hour='{hour_str}')"
            # 尝试删除分区（如果存在，不会删除数据）
