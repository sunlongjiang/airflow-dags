...
if __name__ == "__main__":
    # 创建 Spark 会话
    spark = SparkSession.builder.appName("block_blast_ios_block_action_block_di_pre_0").enableHiveSupport().getOrCreate()
    ...
    tbl_path = f"""temp_dws_block_blast_ios_block_action_di_pre_0_{F_DT}_{current_time_str}"""
    ...
    USING PARQUET
     location 's3://hungry-studio-tmp/{tbl_path}/'
		...
		lateral view json_tuple(
		    ...
		    `block_shape`,
		    `design_postion_upleft`
     '''.format(DT=DT,tmp_tbl_name=tmp_tbl_name,tbl_path=tbl_path)
    print(exec_sql)
