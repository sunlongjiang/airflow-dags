...
if __name__ == "__main__":
    ...
    exec_sql = '''
insert overwrite table hungry_studio.dws_block_blast_gp_block_revive_round_di
select distinct_id
       ,game_type
       ,round_id
       ,game_id
  ...
  select  
    ...
    ,sum(case when event_name in ('game_popup_revive_click') then 1 else 0 end) as revive_click_cnt
  ...
  from 
    (
      ...
      WHERE 
        dt = '{DT}' 
        AND event_name IN ('game_revive','game_revive_ui_show','game_popup_revive_click', 'usr_data_game_end')
