...
if __name__ == "__main__":
    # 创建 Spark 会话
    spark = SparkSession.builder.appName("block_blast_gp_block_action_round_di_pre").enableHiveSupport().getOrCreate()
    DT = sys.argv[1]
    ...
    tmp_tbl_path = f"""temp_dws_block_blast_gp_block_action_round_di_pre_{F_DT}_{current_time_str}"""
    print(tmp_tbl_name)
    # 删除存在的临时表
    sql_delete_tmp_tbl = '''
...
CREATE TABLE default.{tmp_tbl_name}
     location 's3://hungry-studio-tmp/{tmp_tbl_path}/'
           select 
            device_id,
            app_id,
            install_datetime,
            ...
            ,network_type
            ,revive_cnt 
            ,cast(c.game_way_type as int) as game_way_type
            ,c.user_waynum
            ,cast(c.active_ab_game_way_type as int) as active_ab_game_way_type
            ,c.nowwayarr
            ,c.latest_now_way
            ,weight_list
...
device_id,
  ...
  (select  
            ...
            --修复字段
	        sum(case when is_blast and combo_cnt='1' then 1 else 0 end) game_combo_cnt, -- 段数 修复从combo_cnt='0' 变成 combo_cnt='1'
            ...
            ,dt
            from hungry_studio.dwd_block_blast_gp_block_action_block_di  t where dt='{DT}' --  and distinct_id='B790632A-83FE-4AA9-8066-333409E2A1D7' 
            and block_index_id is not null
            group by 
            device_id,
            app_id,
            install_datetime,
            ...
            revive_cnt as revive_cnt,
            revive_show_cnt as revive_show_cnt,
            revive_click_cnt as revive_click_cnt,
            game_revive_color_list as revive_color_list,
            game_end_color_list as end_color_list 
             from hungry_studio.dws_block_blast_gp_block_revive_round_di where dt = '{DT}' 
            ) b 
     ...
     left join 
            ...
            from hungry_studio.dim_block_blast_gp_user_label_ha where dt = '{DT}' and hour = '23'
            ) c 
    ...
    left join  
            ...
            down_reason as down_reason 
             from hungry_studio.dws_block_blast_gp_game_get_block_end_di where dt = '{DT}' 
            ) d
...
;
     '''.format(tmp_tbl_name=tmp_tbl_name,tmp_tbl_path=tmp_tbl_path,DT=DT)
    print(exec_sql)
    spark.sql(exec_sql)
