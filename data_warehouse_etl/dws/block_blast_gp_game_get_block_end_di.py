...
if __name__ == "__main__":
    # 创建 Spark 会话
    spark = SparkSession.builder.appName("block_blast_gp_game_get_block_end_di").enableHiveSupport().getOrCreate()

    DT = sys.argv[1]
    spark.sql('''set hive.exec.dynamic.partition.mode=nonstrict;''')
    exec_sql = '''
insert overwrite table hungry_studio.dws_block_blast_gp_game_get_block_end_di
SELECT distinct_id
